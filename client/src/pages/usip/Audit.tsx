...
export default function Audit() {
  ...
  const { data, error } = trpc.audit.list.useQuery({ entityType: entityType || undefined, limit: 200 });
      ...
      <div className="p-6 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <select className="text-sm border rounded h-9 px-2" value={entityType} onChange={(e) => setEntityType(e.target.value)}>
            ...
            {["account", "contact", "lead", "opportunity", "customer", "quote", "campaign", "workflow_rule", "social_post"].map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
        </div>
        <Section title={error ? "Forbidden — admin role required" : `Recent activity (${data?.length ?? 0})`}>
          {error ? <EmptyState icon={Activity} title="Access denied" description={error.message} /> :
            (data ?? []).length === 0 ? <EmptyState icon={Activity} title="No audit entries" /> : (
