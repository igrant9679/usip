...
const policyInput = z.object({
  ...
  dailySearchCap: z.number().int().min(POLICY_BOUNDS.dailySearchCap.min).max(POLICY_BOUNDS.dailySearchCap.max).optional(),
...
function policyToRow(p: LinkedInLimitPolicy) {
  return {
    ...
    dailySearchCap: p.dailySearchCap,
    ...
    const rows = await Promise.all(accounts.map(async (a) => {
      ...
      return {
        ...
        usage: {
          ...
          searchesToday: usage.today.search ?? 0,
