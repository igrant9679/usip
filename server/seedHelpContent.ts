...
3. You press **Confirm** on each card (or **Not now**). Only then does anything run, and it runs as you — the same role checks the pages apply. The one place it is *wider* than a page is the read side: the assistant's data lookups are scoped to your workspace rather than to the records you personally own, so it can read across the workspace even where a per-member override is set.
...
5. **Meeting Autopilot** proposes meetings with times and a drafted invite for prospects who look ready; you edit and approve each one before it sends.
...
**Connecting a vendor.** Settings → Data sources → the vendor's card. Paste the key, Save, then **Test connection**. The test records the key's scopes and plan tier and, for WarmySender, captures the search tool's parameter schema — that is how Velocity learns whether the vendor filters on job title natively or only keyword-matches it. For WarmySender also set the **billing anniversary day** (the plan's allowance resets then), and record any **purchased lead credits** (they never expire and skip the daily pace). The card's **Enabled** switch turns the source off workspace-wide without removing the key — campaigns and searches skip it while it is off. The same per-source switches live on ARE Settings; both edit one setting.
...
You rarely create Accounts or Contacts by hand — they're produced by converting a lead. Keep new outbound names as **Prospects** and promote them as they engage.`,
  ...
  },
  {
    ...
    bodyMarkdown: `Your pipeline lives under **Win deals → Deals** — a kanban of opportunities grouped by stage (the default pipeline uses discovery → qualified → proposal → negotiation → won/lost; your stages are whatever Settings → Pipelines defines), with the Deal Autopilot layered on top; the classic board also remains at /pipeline. Drag a card between columns to change its stage, or — keyboard/no-mouse — focus a card (Tab) and use the **◀ / ▶ Move** buttons on it. Cards show value, win probability, and AI next-best-actions; if AI suggests a stage change you'll see an **Accept** chip. Use the view toggle for the **Forecast** rollup. Every stage move is recorded in the opportunity's stage history. A stage counts as a close only when it is ticked **Won** or **Lost** in Settings → Pipelines — that tickbox, not the stage name, is what turns a deal into revenue, into a loss, and out of the open forecast. Keep stages honest — the forecast and alerts depend on it.`,
  },
  {
...
**Where people go.** With **Campaign Routing** on (Autonomy Center), the engine puts people who are in nothing into the campaign that fits them best — suggesting picks in Approve, enrolling them in Auto — and **proposes new campaigns** for the people no active campaign fits, grouped by industry, job family and country. Proposals appear on the Revenue Engine hub; creating one makes the campaign (active, batch approval) and adds those people.`,
  ...
  /* ── Sequences & Email: conversations autopilot + sending prefs ─────────── */
  {
    ...
    bodyMarkdown: `**Conversations** (Engage) is where inbound replies get handled. Three channels sit in the header toggle: **Email** (sequence replies), **Social** (LinkedIn DMs), and **Calls** (AI voice-agent phone calls with transcripts).\n\nThe **reply Autopilot** classifies each inbound reply into one of eight classes (willing to meet, needs info, objection, not interested, unsubscribe, out-of-office, wrong person, other) and acts per class. Modes: **Off** (you do everything), **Approve** (AI classifies and suggests; you apply), **Auto** (AI classifies *and* acts). The flagship autonomous move: a **"willing to meet"** reply automatically sends your self-serve **booking link** — on email, and on social where we messaged that person first — so the prospect books a real calendar slot with zero back-and-forth. Social is scoped on purpose: a LinkedIn DM only counts as a reply (and only gets an automatic answer) when we opened the conversation, so a stranger's cold pitch is listed under **Not our outreach** rather than acted on.\n\nUnsubscribes are suppressed automatically. Out-of-offices genuinely pause and resume: the sequence stops, and the enrollment is scheduled to restart on the return date the auto-reply states (7 days later when it states none, capped at 90). The 5-minute sequence tick wakes it and carries on from the next unsent step — nothing is re-sent. A real reply from that person before then cancels the restart, and after three consecutive out-of-offices we stop probing. Every handled reply is marked on the thread. Use **Classify with AI** to catch up a backlog. Watch the stat cards (unhandled / willing to meet / meetings from replies) to see the loop closing.`,
  },
  {
  ...
  },
  {
    ...
    pageKey: "workflows",
    bodyMarkdown: `**Workflow rules** (sidebar → Automation and analytics → Workflow rules) are event-triggered if-this-then-that automations, separate from the AI autopilots (those live in the Autonomy Center). A rule pairs a **trigger** with one or more **actions**.\n\n**Live triggers (six):** *record created* and *record updated* — pick the scope on the rule: **leads, contacts, opportunities, or any**; leads fire from the CRM, web forms, landing pages, booking links, the chat agent and prospect promotion, and a lead conversion fires all three at once. *stage changed* — an opportunity moves, from any of the five places a stage can change: the Kanban, the Alerts page's move control, an approved stage-change request, and either proposal-accept path (so "when a deal is won" covers an accepted proposal). *task overdue* — a task crosses its due date. *signal received* — a buying signal such as a detected job change. *deal stuck* — no movement past your day threshold, checked nightly; its conditions are honoured, so "stuck **and** over $50k" means both.\n\nAccounts are not a trigger scope yet: update field, enroll in sequence and AI email draft have no account handler, so such a rule would fail most of its actions.

**Conditions follow the record type.** The field list offered is exactly what that event carries: a contact has no *stage*, and a brand-new lead has no *score* — scoring runs after the record is created, so a "new lead with score over 60" rule could never match. Pick the record type first and the condition list follows it. An action the chosen type cannot run — enroll in sequence or AI email draft on an opportunity — is labelled *not supported* rather than silently failing on every fire.\n\n**Actions (eight):** **create task**, **send AI email draft**, **notify user** (in-app), **update field**, **enroll in sequence**, **post to Slack**, **notify Teams**, and call a **webhook**. Conditions let you scope a rule (e.g. only deals over a value, only stage = negotiation).\n\nBulk paths never fire rules — a 5,000-row import, a prospect import or an ARE bulk approval creates records silently, by design, so one import cannot become 5,000 webhooks. There is also a per-minute burst cap per workspace and trigger; a suppressed burst is recorded once in the run history as *skipped*.\n\nUse **Test fire** on any rule to run its actions immediately with sample context — it exercises the exact same code path as the real trigger, so if the test posts to Slack, the real event will too. Every run is recorded in the rule's history with success/failure.`,
  },
  ...
  /* ── Team & Admin ────────────────────────────────────────────────────────── */
  {
    ...
    bodyMarkdown: `Velocity has four roles, in rank order: **Rep → Manager → Admin → Super admin**. Almost every permission question in the product resolves through that one ranking.\n\n**The rank rule:** you can only act on people *below* you. An admin can change a rep's or a manager's role, deactivate them, or delete them — but not another admin's, and not their own. A super admin is exempt and can act on anyone. This is why an action that works on one teammate is silently unavailable on another: check their role first.\n\n**What each role adds:**\n- **Rep** — their own records, sequences and tasks. Private sequences they own are visible only to them and to managers and above.\n- **Manager** — sees every rep's work unfiltered, plus team-wide reporting.\n- **Admin** — Settings, the Team page, invitations, integrations, mailboxes, workflow rules and the autopilots.\n- **Super admin** — everything above, plus the Danger Zone (see "Archiving, transferring and exporting").\n\n**Per-member overrides.** Six capabilities are settled individually rather than by role, on the **Permissions** tab of the member's row on the Team page. Each switch shows the *effective* answer — what the server would say right now — and an override wins over the role default **in both directions**: it can hand a rep something their role would not give them, and it can refuse an admin something their role would.

| Override | Granted by default for | What it gates |
|---|---|---|
| **Export data** | Admins and super admins only | The report CSV download, emailing a saved report (now or on a schedule), the campaign-rejection CSV, and the Danger Zone data export |
| **Manage API keys** | Admins and super admins only | AI credentials, Apollo, QuickEnrich, Reoon and the prospect sources |
| **Manage sequences** | Everyone | Creating, editing, forking, saving the canvas and deleting sequences; sharing one with the team or assigning it; and the A/B variant tab, which edits the subject and body a step actually sends |
| **Manage integrations** | Everyone | Connecting, testing and disconnecting integrations |
| **Access billing** | Everyone | Settings → Billing and credits |
| **View all leads** | Everyone | **Nothing yet** — see below |

**\"View all leads\" is not enforced today.** The switch stores your answer and lead visibility is not narrowed by it. It is listed here rather than hidden because an admin who has already turned it off should know it never took effect. Lead scoping is separate, approved work.

**The exports the switch cannot stop.** *Export data* is enforced on the server wherever Velocity builds the file for you: the **Reports** CSV, **Send now** and email schedules on a saved report, the campaign-rejection CSV, and the Danger Zone export. The per-page Export buttons on Leads, Contacts, Pipeline, the audit log and people selections build their file in your browser out of rows the page has already loaded — Velocity hides those buttons when the permission is off, but that is tidiness, not a boundary. The same is true of the **Run** button in the report builder: reading your own numbers on screen stays open to everyone, and those rows can be copied out of the page.

**Pausing a sequence is always allowed**, even with *Manage sequences* off. Stopping a running sequence is the safety lever, and somebody who can see it sending the wrong thing must be able to stop it.\n\n**Two things the workspace protects on its own**, whatever your rank: it will not let you remove the last remaining super admin, and it will not let you delete the member listed as the workspace **owner**. The owner is the standing recipient for everything the autonomous engines report when no human is present, so deleting them would quietly silence the workspace's own automation. Move it deliberately with **Transfer ownership** first.`,
  },
  {
...
export const TOURS: TourSeed[] = [
  ...
  },
  {
    ...
    roleTags: ["admin"],
    steps: [
      { title: "What is missing, counted", bodyMarkdown: "Missing emails, missing domains, duplicates and stale records, counted rather than guessed at. Three duplicate checks, not one: duplicate People, duplicate contacts, and people split across People and Contacts — the same human with a People record and a CRM contact that were never linked. Linking a split pair points the contact at its People record; it never deletes a row.", routeTo: "/data-health", visualTreatment: "coach", advanceCondition: "next_button" },
  ...
  },
  {
    ...
    roleTags: ["sdr"],
    steps: [
      { title: "A segment is a live rule", bodyMarkdown: "Define the conditions once and membership updates itself as records change. Membership is evaluated over your Contacts, so a lead or a prospect joins once it has been converted.", routeTo: "/segments", visualTreatment: "coach", advanceCondition: "next_button" },
  ...
  },
  {
    ...
    roleTags: ["sdr"],
    steps: [
      { title: "Where a won deal goes", bodyMarkdown: "Moving an opportunity into a stage ticked **Won** creates the customer record automatically. This is the post-sale view: health, renewal risk and expansion.", routeTo: "/customers", targetDataTourId: "page-customers", visualTreatment: "spotlight", advanceCondition: "next_button" },
      { title: "Renewals start here", bodyMarkdown: "A customer's **contract end date** is what feeds the Renewals page — it is the field that decides which column the card sits in. Health scores then tell you which of those need attention early.", visualTreatment: "coach", advanceCondition: "next_button" },
    ],
  },
  {
    name: "Renewals",
    ...
    roleTags: ["sdr"],
    steps: [
      { title: "Upcoming and at risk", bodyMarkdown: "Columns are the ladder the contract end date walks down — Early, then 90 / 60 / 30 days, then **Past due** once the date has gone by with no outcome recorded. Health is the pill on the card, not the column. **Renewed** and **Churned** are only ever reached by recording a contract amendment.", routeTo: "/renewals", targetDataTourId: "page-renewals", visualTreatment: "spotlight", advanceCondition: "next_button" },
  ...
  },
  {
    ...
    roleTags: ["sdr"],
    steps: [
      { title: "Three ways a meeting appears", bodyMarkdown: "You book one manually, Meeting Autopilot proposes times for you to edit and approve, or someone books themselves through your link or the website chat agent. They all land here.", routeTo: "/v2/meetings", targetDataTourId: "meetings-list", visualTreatment: "spotlight", advanceCondition: "next_button" },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Watch the bounces", bodyMarkdown: "Rising bounces mean your list quality slipped, not that your copy got worse. Verify addresses before sending rather than after. The rate is a rolling 30-day window counted per recipient, and a mailbox that has not reached enough recipients shows a dash instead of a number — that is \"no evidence yet\", not a clean bill of health.", visualTreatment: "coach", advanceCondition: "next_button" },
    ],
  },
  {
    ...
    roleTags: ["sdr", "admin"],
    steps: [
      { title: "The funnel end to end", bodyMarkdown: "Sourced, contacted, replied, meetings, closed — the same people the engine sourced, counted at five moments, all time and across every campaign. That line under the heading is the scope, and it is the point: five bands are only a funnel when they measure one population.", routeTo: "/v2/analytics", targetDataTourId: "analytics-overview", visualTreatment: "spotlight", advanceCondition: "next_button" },
      { title: "Why 'Emails transmitted' is bigger", bodyMarkdown: "Contacted counts PEOPLE the engine reached. Emails transmitted counts every message the workspace sent, notifications and test sends included. They are different questions, so they are labelled apart rather than stacked into one funnel.", visualTreatment: "coach", advanceCondition: "next_button" },
      { title: "The campaign funnel answers something narrower", bodyMarkdown: "A campaign's Step performance tab follows one campaign's journeys step by step. It will not match this page band for band unless you are running exactly one campaign, and that is correct — it is a different question, not a disagreement.", visualTreatment: "coach", advanceCondition: "next_button" },
    ],
  },
