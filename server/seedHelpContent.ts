...
|---|---|---|
| **Export data** | Admins and super admins only | The report CSV download and the Danger Zone data export |
| **Manage API keys** | Admins and super admins only | AI credentials, Apollo, QuickEnrich, Reoon and the prospect sources |
| **Manage sequences** | Everyone | Creating, editing, forking, saving the canvas and deleting sequences |
...
**Two exports the switch cannot stop.** *Export data* is enforced on the server for the **Reports** CSV and the Danger Zone export. The per-page Export buttons on Leads, Contacts, Pipeline, the audit log and people selections build their file in your browser out of rows the page has already loaded — Velocity hides those buttons when the permission is off, but that is tidiness, not a boundary.
