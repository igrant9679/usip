...
const ARTICLES: ArticleSeed[] = [
  ...
  },
  {
    ...
    pageKey: "prospects",
    bodyMarkdown: `Have a list already? On **People**, open **Import → Import a CSV** (or use *More → Import contacts*). Map your columns (name, title, company, email, LinkedIn URL) and import. Imported rows appear in your People list with an **email status** badge. CSV-imported prospects start without an ICP-fit score (that's only set by Discovery), so use the email-status filter to find the deliverable ones. From there, select and enroll into a sequence, or run **Find contact info** to verify emails before sending.`,
  },
  {
...
const TOURS: TourSeed[] = [
  {
    ...
    steps: [
      ...
      { title: "Your sidebar", bodyMarkdown: "Your core pages listed flat — People, Companies, Sequences, Emails, Calls, Tasks, Meetings, Deals, and more — with Saved records and a More group below holding everything else.", targetDataTourId: "sidebar-nav", visualTreatment: "spotlight", advanceCondition: "next_button" },
