...
**Data Enrichment** (/v2/data-enrichment) — the hub. *Enrich* fills missing emails, titles and companies (select people → Enrich ▾ → Enrich fully). *Find Prospects* searches new people by role and company shape. *Import Contacts* brings in a CSV with exact-match column mapping and per-destination required fields. *Data Health* shows three separate checks — duplicate People, duplicate contacts, and people split across People and Contacts (one human with a People row and a CRM contact that are not linked) — plus field gaps and import audits. **Duplicate People (merge)** is the People merge: two People rows holding one email collapsed into one, preview first, then confirm. **Merge all N shown** merges every group listed in one confirmed click; the confirm names how many rows will be deleted and every address the LinkedIn pass drops, and any group that changed since the list loaded is refused. The survivor is the row a contact already points at, otherwise the lowest id; it keeps every value it already has and fills its blanks from the rows being deleted, so a phone on one and a city on the other both survive; everything pointing at the deleted rows — contacts, enrolments, drafts, replies, campaign queues, saved lists, tasks, meetings, activity, scores and the LinkedIn dossier — is repointed first, and where only one row per person is allowed the rows are combined into that one rather than dropped. The confirm carries the ids the preview showed: if the rows change while the dialog is open (a new import row, or a contact linked from the section above), the cluster is refused rather than merged onto a different survivor. It is admin-only and **irreversible** — the losing People rows are deleted and only the audit log keeps a copy, which is written before anything is destroyed. **Duplicate People (same LinkedIn profile)** is the same merge run on a second key: it groups rows that name the same linkedin.com/in/ profile (ignoring www., capitals, a trailing slash and tracking parameters) and finds the duplicates the email pass cannot — two rows for one person whose addresses enrichment guessed from different patterns, e.g. tyler@richeymay.com and tyler.house@richeymay.com. Rows with no LinkedIn URL are never grouped, and neither are rows whose URL is a placeholder like /in/unknown or /in/N/A — one of those can be held by dozens of unrelated people. Because those rows hold different addresses the merge drops one: the survivor is the row a contact points at, otherwise the row whose address was verified valid (an invalid one is never preferred, and a row with no address at all never wins on a verdict), otherwise the lowest id — and the verdict stays with the address it describes, so a survivor keeping its own address is never relabelled valid or invalid by the row being deleted — and the dropped address is named in the preview before you confirm and written to the audit log, because Velocity has no second-address field on a person and this merge does not invent one. The **Enrichment sweep** and **Company backfill** cards run backlog email-finding and LinkedIn company backfill on their own dials and daily caps.
...
**Meetings** (/v2/meetings) — proposed, invited, scheduled, completed, no-show. Confirm AI proposals here; a proposal whose offered times have all passed shows a **Regenerate** button (fresh future times, fresh invite — also swept automatically each autopilot tick, and "Regenerate outdated" does the whole queue, 10 per click). Offered times start between 9:00 and 16:00 in the workspace time zone; a proposal holding any time outside that window counts as outdated too. Your **booking link** lives here: set its timezone (it defaults to UTC).
...
| Meeting Autopilot | proposed meetings with candidate times 9:00–16:00, spread across proposals and around the owner's live calendar; edit, regenerate, reassign, approve | Autonomous: sends each new invite as it is drafted, inside the send window; the queue still needs approval |
...
**The send window.** Everything above that sends to prospects on its own (campaign emails, sequence auto-send and LinkedIn DMs, Autonomous invites, reminders, booking-link replies, chat follow-ups) waits for the workspace's send window: Settings → Workspace overview, default 6 AM–5 PM Monday to Friday in the workspace time zone. Held work goes at the first check inside it; nothing is dropped. What a person clicks (Approve & send, Send now) is never held.
...
5. **Replies** land in Conversations; handle them. Willing to meet gets the booking link or proposed times (the reply Autopilot in Auto sends the link inside the send window; the Meeting Autopilot in Autonomous sends invites). A meeting counts as booked only when the prospect accepts the invite.
...
1. Data Health: merge duplicate contacts and companies; merge duplicate People from the Duplicate People (merge) card (preview the plan, then confirm — this is irreversible and deletes the losing rows); link people split across People and Contacts (this LINKS the contact to its People record and merges the contact’s own values into it — it never deletes a row and never merges two People rows); fix import-mapping issues.
