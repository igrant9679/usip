...
- [x] Migration: run drizzle-kit generate and apply SQL via webdev_execute_sql
- [x] Server: create server/routers/proposals.ts with list, get, create, update, updateStatus, updateSection, upsertMilestone, deleteMilestone, submitFeedback (public), getByShareToken (public), generateSectionContent (AI)
- [x] Server: register proposalsRouter in server/routers.ts
- [x] UI: add Proposals nav entry to Shell.tsx (Operate group)
- [x] UI: add /proposals and /proposals/:id routes to App.tsx
- [x] UI: create Proposals list page (client/src/pages/usip/Proposals.tsx) with dashboard stats + proposal cards + New Proposal wizard (with Contacts/Accounts pre-fill)
- [x] UI: create ProposalDetail page (client/src/pages/usip/ProposalDetail.tsx) with Overview, Content, Timeline, Process, Feedback, Share tabs
- [x] UI: public client portal route /p/:shareToken (client/src/pages/ClientPortal.tsx)
- [x] UI: register public /p/:shareToken route in App.tsx
...
## Phase A — Enhancements (round 16)
- [x] Server: proposals.getExtensionHistory public procedure — returns extension-related activities for a shareToken
- [x] Server: proposals.listExtensionPending protected procedure — returns proposals with pending extension requests (request but no approve/deny)
- [x] Server: operations.ts proposal_expiry_funnel resolver — windowDays added to each series item for drill-down URL construction
- [x] UI: ProposalPortal — "Extension History" tab (read-only timeline with color-coded dots for request/approve/deny events)
- [x] UI: DashboardWidgets ProposalExpiryFunnelWidget — each bar segment and legend item is a clickable link to /proposals?expiryFilter=X&window=Y
