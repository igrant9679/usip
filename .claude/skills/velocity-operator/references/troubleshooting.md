...
| Signed out sooner than expected, mid-day | Settings → Security → Session timeout is now ENFORCED (2026-09-20) and is the ABSOLUTE age of a sign-in, not idle time. A member in two workspaces gets the shorter of the two | Raise the number; it lands at each member's next sign-in, not immediately |
| Whole app replaced by "Two-factor authentication required" | Settings → Security → Enforce 2FA is on and this member has no authenticator app | Connect one on the panel. Lost phone: an admin resets 2FA from the Team page — no backup codes. A super admin is never blocked |
| Admin cannot switch Enforce 2FA on ("Connect your own authenticator app first") | Deliberate: `settings.save` is itself behind the gate, so an unenrolled admin turning it on would lock themselves out | Settings → Profile → Multi-factor authentication, then save |
| IP allowlist saved but anyone can still sign in | Working as designed — the allowlist is stored and NOT enforced, and the page says so in amber. No trusted-proxy hop count is configured, so the app cannot tell a real address from a claimed one | Not a bug. Needs `app.set('trust proxy', n)` verified against a real request first |
| A rep gets FORBIDDEN editing or creating a sequence | `manage_sequences` override is off for them (the old "Rep" template button used to write it) | Team page → member → Permissions. Note pausing a sequence is never gated; everything else is, including the A/B variant tab, sharing a sequence with the team, and assigning one |
...
| Export CSV missing on /reports | `export_data` — denied by default for manager and rep | Same place. Also refuses "Send now" / scheduling a saved report, and the campaign-rejection CSV. The Leads/Contacts/Pipeline/Audit Export buttons are hidden by the same flag, but that is UX only: those files are built in the browser |
