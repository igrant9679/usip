...
- **Sequence template** (`sequenceTemplate`): how many steps the skeleton has — `standard_7step` (7), `aggressive_3step` (3), `nurture_14step` (14), `custom` (5). The workspace default (ARE Settings) seeds NEW campaigns only; there is no per-campaign picker in the UI, so changing an existing campaign's template goes through the AI assistant or the API. Changing it clears the cached skeleton so the next generation uses the new count.
- **Cadence**: `stepGapDays` (default 7) spaces the steps — the campaign owns the schedule, not the generated `day` values. A 7-step template therefore runs six weeks and a 14-step one about three months. Channels email / LinkedIn; per-step A/B variants.
