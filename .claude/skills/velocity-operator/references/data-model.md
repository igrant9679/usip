...
| **Contact** | `contacts` | Companies → people, People drawer | The durable person on an Account. Has `personProspectId` back-link to the People row. A contact whose back-link is null, or points at a different People row than the one holding its email, is detectable and repairable from Data Health → Split across People & Contacts (the repair links, never deletes). Two People rows holding one email are resolved by the People merge instead — see below. | Lead conversion, import, campaign reply promotion |
...
### Merging two People rows (irreversible)

Two People rows holding the same usable, non-generic email are a duplicate cluster. Data Health → **Duplicate People (merge)** plans and then applies a merge (`server/services/personMerge.ts`, admin-only).

There are **two passes over the same merge**, not two merges:

- **Duplicate People (merge)** — groups on the shared **email address**. This is the pass that ran against production on 2026-09-20 (169 clusters, 182 rows deleted, 196 references repointed).
- **Duplicate People (same LinkedIn profile)** — groups on the normalised **`linkedin.com/in/<slug>`**, ignoring protocol, `www.`/country subdomain, case, a trailing slash, a query and a hash. It finds the duplicates the email pass cannot: two rows for one human whose addresses were *guessed* by enrichment from different patterns (`tyler@richeymay.com` and `tyler.house@richeymay.com`). A row with no usable profile URL is never clustered — an empty key would fuse every person without a LinkedIn into one cluster — and neither is a row whose URL holds a **placeholder slug** (`/in/unknown`, `/in/N%2FA`, `/in/null`): enrichment stores those by the dozen and they all normalise to one key, so the plan reports them as `skippedPlaceholderProfiles` instead of clustering strangers.

- **What survives**, in order — (a) the row a contact already points at (`contacts.person_prospect_id`); (b) in the LinkedIn pass only, the row whose `email_status` is `valid`; (c) then `accept_all`, then `risky`/`unknown`/no verdict, then `invalid`, and last of all a row holding **no usable address** — a verdict describing no address is not a better address, so such a row never wins (b)/(c) and is never reported as "its address is the verified one"; (d) then the **lowest id**. Never `updatedAt`, which churns on every enrichment sweep. (a) beats (b) deliberately: a contact-linked row holding an *invalid* address still survives, because the link is a statement a human made about which row this person is and the address is the recoverable half. The email pass does not use (b)/(c) at all — every row in an email cluster holds the same address, so ordering on the verdict could only churn which row dies.
- **What happens to the other address** — in the LinkedIn pass the rows hold *different* addresses and the survivor keeps its own, so one real address stops existing. Every dropped address is listed as `discardedEmails` in the plan (shown in the preview before the confirm), in the result, and on the audit row. It is **not** written to `catch_all_email`: that column means "the generic inbox this person's address replaced" and the People UI labels it that way, so a personal pattern-guess does not belong there, and no column is invented for it.
- **The verdict travels with the address** — `email_status`, `email_verified_at` and `email_revealed_at` describe ONE address, so they are only ever filled from a row holding the address the survivor ends up with. A survivor keeping its own address keeps its own verdict (blank included); a survivor whose address column holds an unmailable placeholder (`<UNKNOWN>`, a malformed address) counts as having none and inherits the cluster's best real address instead of keeping the placeholder for good.
- **What the survivor keeps** — every non-empty value it already has. Its blanks are filled from the rows being deleted, lowest id first, so a phone on one row and a city on the other both come through. `0` and `false` are values, not blanks. A non-empty survivor value is never overwritten.
- **What moves** — every reference to a deleted row is repointed at the survivor first. Fifteen ordinary id columns: contacts, enrollments, email drafts, campaign routing suggestions, contact import rows, email replies, the campaign prospect queue, prospect field history, the three LinkedIn enrichment tables, LinkedIn job items, both `linkedin_enrichment_batch_rows` prospect columns and `prospect_search_results.promoted_prospect_id`. Nine POLYMORPHIC ones, which a column-name scan cannot see: saved-list membership (`record_list_members`, `record_type='prospect'`), the three scored tables (`score_results`, `score_history`, `priority_score_results`, `object_type='person'`), and tasks, meetings, activities, calendar events and voice calls (`related_type='prospect'`). Plus the `prospect_ids` array on campaign proposals, rewritten element-wise. Several contacts pointing at one survivor afterwards is expected and correct; de-duplicating contacts is a separate action.
- **What is combined rather than moved** — where only one row per person is allowed, the cluster's rows are field-unioned into the richest one and the emptied duplicates are deleted: the LinkedIn dossier (`prospect_linkedin_enrichments`, whose `enrichment_id` children follow the row that survives), one score per model, one place per saved list. A stub never beats a full dossier.
- **What is refused** — a cluster whose rows disagree on `last_name` or `company_domain` is reported as skipped, not merged: two different humans can share one mailbox, and the same guard applies to a shared LinkedIn profile. Generic inboxes (info@, sales@…) are never clustered at all. So is a cluster whose rows no longer match the ids the preview showed — the confirm carries `{survivorId, loserIds}` and the server compares them against a fresh plan, because linking a contact from the section above changes which row the survivor rule picks.
- 🔴 **IRREVERSIBLE.** The losing rows are DELETED. The only remaining copy is the `audit_log` row written per merged cluster (`entity_type = person_merge`), which carries the deleted rows, every row deleted as a duplicate, the survivor, the fields filled, **every discarded email address** and the per-table repoint counts. It is written BEFORE the first destructive statement, with a direct insert rather than `recordAudit` — a record that fails silently after the delete is no record at all.
...
- `sequenceTemplate`: `standard_7step` (7 steps) | `aggressive_3step` (3) | `nurture_14step` (14) | `custom` (5). Free text at the API, so an unknown value falls back to 5 (`shared/areSequenceTemplates.ts`). Only read on the `per_person` path — a `fixed` campaign's step count comes from its own `fixedSteps`.
- `stepGapDays`: days between consecutive steps, default 7, clamped to 1–30. The campaign owns the schedule; the generated `day` values are copy guidance only.
...
- **Saved searches** (`saved_searches`, migration 0185): a per-user saved QUERY for a record page — columns + filters + sort in one `config` json blob, plus `lastAppliedAt` so the picker comes back where you left it. Private to the owner (`workspaceId` + `ownerUserId`); sharing is not built. Only People writes them today (`surface = 'people'`). A LIST is a set of records, a SAVED SEARCH is a query — one word apart, routinely confused.
...
- **ICP profile** (`icp_profiles`, versioned): the living ideal-customer profile; regenerated by the ICP cron on the workspace's configured ICP Re-inference Schedule (`workspace_settings.areIcpRegenSchedule` — `daily` default / `weekly` / `on_new_deal` / `manual`, where `manual` skips the workspace entirely) from deals in stages flagged Won (and, for the negative signal, stages flagged Lost) plus engagement; drives discovery targeting and the campaign screen floor.
