...
Every Off / Approve / Auto dial on one screen: Task, Meeting, Conversation, Deal, Social, Job change, Chat agent, Chat follow-up, Enrichment sweep, Company backfill, Campaign routing, Optimisation, Email AI auto-send. "All: Approve" and "All: Off" act on every dial. Also shows AI-proposed workflow rules to adopt. Admin-only to change. An admin can also arm most of these from the AI Assistant (a confirm card that names the dial); Social, Job change and Email AI auto-send are Autonomy-Center-only. See `automation.md`.
...
The traditional stage board over the same opportunities as Deals, back on the rail 2026-09-20 (it had lost its `toolRegistry` entry, never its route). What only this page has: the **Forecast** view (`opportunities.forecast` — weighted and committed projections by stage, with AI commentary you can *generate*, not just read), **New opportunity** with an account picker, **Export CSV**, and a switcher across every pipeline from `/settings/pipelines`. Deals is the complement: Deal Autopilot and the Pipeline Alerts strip live there, not here.
...
### ★ Emails — `/v2/emails`
Every email sitewide: campaign and sequence steps, CRM sends, inbox mail, proposals, system mail, inbound replies. Filters by source, status, account. **AI Pipeline** (`?status=awaiting&source=ai_draft`) and **Email Drafts** (`?status=awaiting&source=sequence`) are saved filters of this page: approve, edit, discard. Clicking a row opens the drawer with the full editor — subject A/B + spam analysis, the AI's research context, regenerate presets, effectiveness score, resolved-merge preview, delivery analytics (the retired /ai-pipeline and /email-drafts pages, folded in 2026-09-15). Header buttons: **AI compose** (run the pipeline for chosen contacts) and **Send approved** (confirm-gated).

### ★ Conversations — `/v2/conversations`
Inbound replies that need a human: classified (willing to meet, question, referral, OOO, left company, not interested, unsubscribe, other) with sentiment, confidence and a suggested reply. Handle = reply, book, task, or dismiss; handling stops the sequence for that person — except an out-of-office, where handling it *schedules the restart* (stated return date, else +7d, capped at +90d; the paused enrollment shows "Resumes &lt;date&gt;" on the Sequences page). Social replies have their own list, scoped to outreach we started — a DM counts as a reply only where we sent in that chat, DM'd that provider id, or invited it (2026-09-20); everything else is listed under the **Not our outreach** filter, never classified, never counted and never auto-answered. Stats: total, unhandled, willing-to-meet, needs-classify.
...
**Security** (2026-09-20) — two of its three controls now enforce, one deliberately does not:
- **Session timeout** — the ABSOLUTE age of a sign-in, not idle time. Applied at each member's NEXT sign-in (the JWT `exp` and the cookie `maxAge` are set from it in `passwordAuth`), so lowering it never signs the team out on the spot. A member in two workspaces gets the SHORTER of the two. Every row was reset to 10080 (7 days) by migration 0186, because the stored values pre-dated any enforcement and were nobody's deliberate choice.
- **Enforce 2FA** — one gate in `workspaceProcedure`. A member with no authenticator app gets a full-screen "Two-factor authentication required" panel (setup dialog + Sign out) and nothing else loads. A super admin is never blocked; an admin cannot save the switch before connecting their own authenticator; the four `profile.*Totp*` / `getMfaStatus` procedures stay reachable; a db fault SUSPENDS the policy rather than locking the workspace out. A lost phone is an admin reset from the Team page (`team.resetMemberMfa`) — there are no backup codes.
- **IP allowlist** — STORED AND OFF BY DESIGN, with an amber notice saying so beside the field. Nothing in `server/` sets `trust proxy`, so `x-forwarded-for` is caller-controlled at whichever hop we picked: near hop waves through anyone who sets a header, far hop locks out every member including the admin who would turn it off. The field shows the address the current request arrived from, and Team → member → sign-in history lists real sign-in addresses, so the list is authorable the day the hop count is settled.
