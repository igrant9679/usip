...
Chat over your own data. It can search people/companies, list sequences/lists/campaigns, read the deals pipeline, run read-only queries over every core table, look up help articles, and hand you links. It can **propose** actions (enroll, tasks, add to list, enrich, pause/activate or draft a campaign, propose meetings, create a list from a filter, set a company brand, update or archive prospects) which run only when you press Confirm in the chat. It cannot send email or LinkedIn messages. It knows which page you opened it from.
